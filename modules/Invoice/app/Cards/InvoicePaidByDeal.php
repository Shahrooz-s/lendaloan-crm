...
class InvoicePaidByDeal extends TableCard {
    /**
    ...
    public function items(Request $request): iterable
    {
        ...
        return Invoice::select([
            (new Deal())->getTable() . '.name as name',
            DB::raw('SUM(CASE WHEN ' . DB::getTablePrefix() . (new Invoice())->getTable() . '.status = "paid" THEN 1 ELSE 0 END) as paid_count'),
            DB::raw('SUM(CASE WHEN ' . DB::getTablePrefix() . (new Invoice())->getTable() . '.status = "paid" THEN total ELSE 0 END) as paid_amount'),
            DB::raw('SUM(total) as forecasted_amount')
        ])
            ->join((new Deal())->getTable(), (new Invoice())->getTable() . '.deal_id', '=', (new Deal())->getTable() . '.id')
            ...
            ->groupBy('deal_id', (new Deal())->getTable() . '.name')
            ->get()
    ...
    public function fields(): array
    {
        ...
        return [
            ['key' => 'name', 'label' => __('deals::deal.deal')],
    ...
    {
        return __('invoice::invoice.cards.invoice_paid_by_deal');
    }
    ...
    public function jsonSerialize(): array
    {
        return array_merge(parent::jsonSerialize(), [
            'helpText' => __('invoice::invoice.cards.invoice_paid_by_deal_info'),
        ]);
    }
}
