...
class ModuleInitializationService
{
    ...
    {
        return config('invoice.verification.evanto_product_id');
    }
    ...
    public function registerCode(string $moduleName, string $activationCode, $evantoRes): array
    {
        ...
        try {
            ...
            $response = Http::withHeaders(['Accept' => 'application/json'])->post(config('invoice.verification.register_code_url'), $data);
            ...
            if (!empty($return)) {
                ...
                settings()->set(["{$moduleName}_license_type" => $evantoRes->license])->save();
            ...
            Log::error('ERROR DURING MODULE ACTIVATION: ' . $e->getMessage());
    ...
    public function registrationFailed($moduleName, $status, $activationCode): array
    {
        ...
        settings()->set(["{$moduleName}_heartbeat" => base64_encode(json_encode(['status' => $status, 'id' => $activationCode, 'end_point' => config('invoice.verification.register_code_url')]))])->save();
