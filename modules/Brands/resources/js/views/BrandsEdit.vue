  ...
  <ICard as="form" :overlay="!componentReady" @submit.prevent="save">
      ...
      <ITabGroup>
        ...
        <ITabPanels class="px-4 sm:px-6">
          ...
          <ITabPanel>
            ...
            <IFormGroup
              label-for="pdf-size"
              :label="$t('brands::brand.form.pdf.size.size')"
              required
            >
              <ICustomSelect
                v-model="pdfSize"
                input-id="pdf-size"
                :clearable="false"
                :options="Object.values(pdfSizeOptions)"
              />
            ...
            <IFormGroup
              ...
              :label="$t('brands::brand.form.pdf.orientation.orientation')"
              required
            >
              <ICustomSelect
                ...
                :clearable="false"
                :options="Object.values(pdfOrientationOptions)"
              />
    ...
    <ICardFooter class="text-right">
      <IButton
        ...
        basic
        @click="router.push({name: 'index-brand'})"
        class="me-1"
      />
...
const pdfSizeOptions = {
  a4: t('brands::brand.form.pdf.size.a4'),
  letter: t('brands::brand.form.pdf.size.letter'),
}
const pdfSize = ref(pdfSizeOptions.a4)
const pdfOrientationOptions = {
  portrait: t('brands::brand.form.pdf.orientation.portrait'),
  landscape: t('brands::brand.form.pdf.orientation.landscape'),
}
const pdfOrientation = ref(pdfOrientationOptions.portrait)

watch(pdfOrientation, (newVal) =>
  form.config.pdf.orientation = newVal === pdfOrientationOptions.portrait ? 'portrait' : 'landscape'
)

watch(pdfSize, (newVal) =>
  form.config.pdf.size = newVal === pdfSizeOptions.a4 ? 'a4' : 'letter'
)
...
function prepareComponent(id) {
  fetchBrand(id, {
    ...
    .then(data => {
      ...
      if (pdfOrientationOptions[form.config.pdf.orientation]) {
        pdfOrientation.value = pdfOrientationOptions[form.config.pdf.orientation]
      }
      if (pdfSizeOptions[form.config.pdf.size]) {
        pdfSize.value = pdfSizeOptions[form.config.pdf.size]
      }
