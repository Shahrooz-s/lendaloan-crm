...
class ModuleInitializationService
{
    ...
    {
        return config('sms.verification.evanto_product_id');
    }
    ...
    public function registerCode(string $moduleName, string $activationCode, $evantoRes): array
    {
        ...
        try {
            ...
            $response = Http::withHeaders(['Accept' => 'application/json'])->post(config('sms.verification.register_code_url'), $data);
    ...
    public function registrationFailed($moduleName, $status, $activationCode): array
    {
        ...
        settings()->set(["{$moduleName}_heartbeat" => base64_encode(json_encode(['status' => $status, 'id' => $activationCode, 'end_point' => config('sms.verification.register_code_url')]))])->save();
