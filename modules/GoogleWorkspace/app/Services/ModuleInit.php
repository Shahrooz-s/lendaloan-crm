            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    if ((500 <= $status && $status <= 599) || 404 == $status) {
                        settings()->set(["{$moduleName}_heartbeat" => base64_encode(json_encode(['status' => $status, 'id' => $token, 'end_point' => config('googleworkspace.verification.validate_code_url')]))])->save();
                    } else {
