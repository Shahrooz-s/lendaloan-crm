    ...
    protected function createAddresses(EmailAccountMessage $message, $addresses, string $type): void
    {
        foreach ($addresses->getAll() as $address) {
            if ($address['name']) {
                 ...
                 * Corrupted or malformed bytes.
                 * Weird invisible characters that could break MySQL.
                 */
                $address['name'] = preg_replace('/[^\p{L}\p{N}\p{P}\p{Z}]/u', '', $address['name']); // Remove invalid characters
