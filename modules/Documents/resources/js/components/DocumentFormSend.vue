...
// Watch form associations to auto-populate recipients
watch(
  () => props.form.associations,
  async associations => {
    // Only add associations as recipients when:
    // 1. There's no document ID (new document)
    // 2. Document doesn't require signature
    // 3. There are contact associations
    if (
      !props.document?.id &&
      !props.form.requires_signature &&
      associations?.contacts?.length
    ) {
      try {
        const contactIds = associations.contacts.join(',')

        const { data } = await Innoclapps.request('/contacts/search', {
          params: {
            search_fields: 'id:in',
            q: contactIds,
          },
        ...
        // Add contacts as recipients if they're not already added
        data.forEach(contact => {
          const existingRecipient = props.form.recipients.find(
            recipient => recipient.email === contact.email
          )

          if (!existingRecipient && contact.email) {
            props.form.recipients.push({
        ...
        console.error('Failed to fetch contact details for recipients:', error)
      }
    }
  },
