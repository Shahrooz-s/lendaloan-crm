...
class DocumentContent implements Stringable
{
    ...
    protected function prepareHtmlForPdf(string $html): static
    {
        ...
        foreach ($dom->find('table') as $table) {
            if (! isset($table->attr['style'])) {
                continue;
            }

            $style = $table->attr['style'];

            // Check if display:flex exists
            if (preg_match('/\bdisplay\s*:\s*flex\b/i', $style)) {
                // Remove display:flex
                $style = preg_replace('/\bdisplay\s*:\s*flex\s*;?/i', '', $style);
                $style = trim($style);

                // Append flex override
                $style = rtrim($style, ';').';flex:0 0 auto;';
                $table->attr['style'] = trim($style, " ;\t\n\r\0\x0B");
            }
        }
